...
class _VendorDataDetailScreenState extends State<VendorDataDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (mounted) {
        setState(() {
          _hasProducts =
              controller.vendorDetail?.vendorMaterials?.isNotEmpty ?? false;
        });
      }
    });
  }
  ...
  // Service-only vendors: Services · About · Reviews · Photos · Brands
  // Vendors with products: Products · Services · About · Reviews · Photos · Brands
  List<_Sec> get _secs {
    return [
      if (_hasProducts) _Sec('Products', Icons.inventory_2_rounded),
      _Sec('Services', Icons.build_circle_rounded),
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ServiceController>(builder: (ctrl) {
      if (ctrl.isVendorDetailLoading) {
        return Scaffold(
          backgroundColor: _bg,
          body: const Center(child: GifLoader()),
        );
      }

      final v      = ctrl.vendorDetail;
      final brands = ctrl.brandList;

      if (v == null) {
        return Scaffold(
          backgroundColor: _bg,
          appBar: _miniBar('Vendor'),
          body: _empty('Vendor not found', Icons.store_outlined),
        );
      }

      final imgUrl  = (v.thumbnail?.isNotEmpty ?? false)
          ? v.thumbnail!
          : (v.image.isNotEmpty ? v.image.first : '');
      final rating  = double.tryParse(v.reviewsAvgRating ?? '0') ?? 0;
      final initials = v.name.trim().split(' ').take(2)
              ...
              child: Container(
                ...
                child: Column(children: [
                  ...
                  Padding(
                    ...
                    child: _buildContactCard(v),
                  ),
                  Padding(
                    padding: const EdgeInsets.fromLTRB(14, 10, 14, 0),
                    child: _buildWriteReviewCard(ctrl),
                  ),
                  Padding(
                  ...
                  Padding(
                    ...
                    child: _buildActiveSection(v, brands, rating, ctrl),
                  ),
                ]),
              ),
            ),
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Container(width: 54, height: 54,
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                colors: [Color(0xFFD4921A), Color(0xFFF5C842)],
          ...
          Expanded(child: Column(mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(v.name, style: const TextStyle(color: Colors.white,
        ...
        child: Column(children: [
          ...
          Row(children: List.generate(5, (i) => GestureDetector(
            ...
            child: Padding(padding: const EdgeInsets.only(right: 3),
              child: Icon(i < ctrl.selectedRating
          ...
          const SizedBox(height: 10),
          TextField(
            controller: ctrl.reviewController, maxLines: 3,
          ...
          SizedBox(width: double.infinity, height: 42,
            child: ElevatedButton(
              ...
              child: ctrl.isSubmittingReview
                  ...
                  : const Text('Submit Review', style: TextStyle(
        ...
        itemBuilder: (_, i) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(secs[i].icon, size: 13,
                    color: active ? Colors.white : _muted),
                const SizedBox(width: 5),
                Text(secs[i].label, style: TextStyle(fontSize: 12,
                    fontWeight: FontWeight.w700,
                    color: active ? Colors.white : _muted)),
              ]),
            ),
          ).animate(delay: Duration(milliseconds: i * 45))
              .fadeIn(duration: 300.ms).slideX(begin: 0.15);
        },
      ),
    );
  }
    ...
    return Column(children: items.asMap().entries.map((e) {
      ...
      return Container(
        ...
        child: Row(children: [
          GestureDetector(
            ...
            child: ClipRRect(borderRadius: BorderRadius.circular(12),
              child: m.image.isNotEmpty
                  ? Image.network(m.image.first, width: 60, height: 60,
                  fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) =>
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: _greenBg, borderRadius: BorderRadius.circular(6)),
          ...
          _pillBtn('Quote', _gold, _goldBg,
                  () => _productDialog(m.materialName, m.id.toString())),
        ]),
    ...
    return Column(children: items.asMap().entries.map((e) {
      ...
      return Container(
        ...
        child: Row(children: [
          GestureDetector(
            ...
            child: ClipRRect(borderRadius: BorderRadius.circular(12),
              child: s.image.isNotEmpty
                  ? Image.network(s.image.first, width: 60, height: 60,
                  fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) =>
          ...
          const SizedBox(width: 8),
          _pillBtn('Book', _green, _greenBg,
                  () => _serviceSheet(s.id.toString())),
        ]),
      ).animate(delay: Duration(milliseconds: i * 50))
          .fadeIn().slideY(begin: 0.08);
    }).toList());
  }
  ...
  Widget _aboutSection(svcModel.Vendor? v) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text((v?.description?.isNotEmpty ?? false)
          ? v!.description!
  ...
  Widget _factChip(IconData icon, String label, String val,
      Color c, Color bg) =>
      Container(
  ...
  Widget _reviewsSection(svcModel.Vendor? v, double rating) {
    ...
    return Column(children: [
      ...
      ...reviews.asMap().entries.map((e) {
        ...
        final ini  = user.name.isNotEmpty
        ...
        return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              CircleAvatar(radius: 18, backgroundColor: _goldBg,
                ...
                child: user.avatar.isEmpty
                    ...
                    fontWeight: FontWeight.w700, color: _gold, fontSize: 12))
                    : null,
              ),
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Container(
                ...
                decoration: BoxDecoration(
                    color: _goldBg, borderRadius: BorderRadius.circular(10)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  const Icon(Icons.star_rounded, color: _gold, size: 11),
                  const SizedBox(width: 2),
                  Text(uRat.toStringAsFixed(1), style: const TextStyle(
            ...
            Text(r.review, style: const TextStyle(
                fontSize: 12.5, color: _muted, height: 1.65)),
          ]),
        ).animate(delay: Duration(milliseconds: 80 + i * 55))
            .fadeIn(duration: 300.ms).slideY(begin: 0.06);
      }),
    ]);
  }
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(b.name ?? 'N/A', textAlign: TextAlign.center, maxLines: 2,
                  overflow: TextOverflow.ellipsis, style: const TextStyle(
                      fontSize: 10.5, fontWeight: FontWeight.w600, color: _ink)),
            ]),
          ).animate(delay: Duration(milliseconds: i * 40))
              .fadeIn().scale(begin: const Offset(0.88, 0.88),
              curve: Curves.easeOutBack, duration: 320.ms);
        },
      ),
  ...
  void _productDialog(String title, String materialId) {
    showDialog(context: context, builder: (_) => GetBuilder<ServiceController>(
      builder: (ctrl) => Dialog(
        insetPadding: const EdgeInsets.symmetric(horizontal: 20),
        backgroundColor: _card, surfaceTintColor: _card,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        child: Padding(padding: const EdgeInsets.fromLTRB(18, 10, 18, 20),
              ...
              Container(
                ...
                child: Row(children: [
                  _stepBtn(Icons.remove, () {
                    if (ctrl.quantity > 1) {
                      ctrl.quantity--;
                      ctrl.quantityController.text = ctrl.quantity.toString();
                      ctrl.update();
                    }
                  }),
                  SizedBox(width: 44, child: TextField(
                    ...
                    onChanged: (val) {
                      ctrl.quantity = int.tryParse(val) ?? 1; ctrl.update();
                    },
                  )),
                  _stepBtn(Icons.add, () {
                    ctrl.quantity++;
              ...
              Expanded(child: Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: ctrl.isUnitLoading
                    ...
                    value: ctrl.selectedUnit,
                    items: ctrl.materialUnits.map((u) => DropdownMenuItem(
                        value: u.id.toString(),
                        child: Text(u.name ?? '',
              ...
              child: ElevatedButton(
                ...
                child: ctrl.isSubmittingEnquiry
                    ...
                    child: CircularProgressIndicator(strokeWidth: 2,
                        color: Colors.white))
                    : const Text('Send Enquiry', style: TextStyle(
  ...
  // ── Service enquiry sheet ──────────────────────────────────────────────────
  void _serviceSheet(String serviceId) {
    Get.bottomSheet(
      GetBuilder<ServiceController>(builder: (ctrl) => Container(
          ...
          children: [
            Center(child: Container(width: 36, height: 4,
                ...
                decoration: BoxDecoration(color: _border,
                    borderRadius: BorderRadius.circular(10)))),
            const Text('Request Service', style: TextStyle(
                fontSize: 16, fontWeight: FontWeight.w800, color: _ink)),
            const SizedBox(height: 12),
            TextField(controller: ctrl.quoteController, maxLines: 3,
              style: const TextStyle(fontSize: 13),
              decoration: InputDecoration(
                hintText: 'Tell us your requirements…',
                filled: true, fillColor: _bg, contentPadding: const EdgeInsets.all(12),
            ...
            const SizedBox(height: 12),
            const Text('Preferred Date & Time', style: TextStyle(
                fontSize: 12, fontWeight: FontWeight.w700, color: _ink)),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: _selTile(ctrl.selectedDate ?? 'Date',
                  Icons.calendar_today_rounded, () async {
                    ...
                    if (d != null) {
                      ctrl.selectedDate = d.toString().split(' ')[0];
                      ctrl.update();
                    }
                  })),
              const SizedBox(width: 10),
              Expanded(child: _selTile(ctrl.selectedTime ?? 'Time',
                  Icons.access_time_rounded, () async {
                    final t = await showTimePicker(context: Get.context!,
                        initialTime: TimeOfDay.now());
                    if (t != null) {
                      ctrl.selectedTime = t.format(Get.context!);
                      ctrl.update();
                    }
                  })),
            ]),
            ...
            SizedBox(width: double.infinity, height: 46,
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: _ink,
                ...
                child: ctrl.isSubmittingEnquiry
                    ...
                    child: CircularProgressIndicator(strokeWidth: 2,
                        color: Colors.white))
                    : const Text('Send Request', style: TextStyle(
    ...
    showDialog(context: context, barrierColor: Colors.black,
      builder: (_) => Scaffold(
        ...
        body: Center(child: GestureDetector(
          onDoubleTap: () {
            tc.value = tc.value != Matrix4.identity()
                ? Matrix4.identity() : (Matrix4.identity()..scale(2.5));
          },
          child: InteractiveViewer(transformationController: tc,
  ...
  // ── Small helpers ──────────────────────────────────────────────────────────
  AppBar _miniBar(String t) => AppBar(
    backgroundColor: _h1, elevation: 0,
  ...
  Widget _pillBtn(String label, Color c, Color bg, VoidCallback tap) =>
  ...
  Widget _stepBtn(IconData icon, VoidCallback tap) => InkWell(
  ...
  Widget _imgBox(double s, IconData icon) => Container(
      width: s, height: s,
      decoration: BoxDecoration(color: _bg, borderRadius: BorderRadius.circular(10)),
  ...
  Widget _empty(String msg, IconData icon) => Padding(
    ...
    child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 10),
      Text(msg, style: const TextStyle(color: _muted, fontSize: 12.5,
