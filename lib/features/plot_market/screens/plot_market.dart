          ...
          Obx(() {
            if (controller.searchQuery.value.isNotEmpty)
              return GestureDetector(
                ...
                child: Icon(Icons.close, color: Colors.grey.shade500, size: 18.w),
              );
...
// Modern Filter Sheet with Range Sliders
...
class _ModernFilterSheetState extends State<_ModernFilterSheet> {
  ...
  void _initFilters() {
    ...
    double startA = _areaChanged ? double.parse(c.minAreaSqft.value) : c.areaMin.value;
    double endA = c.maxAreaSqft.value.isNotEmpty ? double.parse(c.maxAreaSqft.value) : c.areaMax.value;
      ...
      builder: (_, scrollController) => Container(
        ...
        child: Column(
          children: [
            ...
            Container(width: 45.w, height: 4.h, decoration: BoxDecoration(color: Colors.grey[200], borderRadius: BorderRadius.circular(10.r))),
            _buildHeader(),
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  _buildAnimatedSection(1, "PLOT TYPE",
                      icon: Icons.grid_view_rounded,
                      color: Colors.blueAccent,
                      child: _buildPlotTypeChips()
                  ),
                  const SizedBox(height: 32),
                  _buildAnimatedSection(2, "PRICE RANGE",
                      icon: Icons.payments_rounded,
                      color: Colors.green,
                      trailing: _priceChanged ? "₹${_formatNumber(_priceRange.start)} - ₹${_formatNumber(_priceRange.end)}" : "Any Price",
                      child: _buildPriceSlider()
                  ),
                  const SizedBox(height: 32),
                  _buildAnimatedSection(3, "AREA RANGE",
                      icon: Icons.square_foot_rounded,
                      color: Colors.purple,
                      trailing: _areaChanged ? "${_areaRange.start.toInt()} - ${_areaRange.end.toInt()} sqft" : "Any Area",
            ...
            children: [
              Row(
                children: [
                  ...
                  SizedBox(width: 8.w),
                  Text(title, style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.w800, letterSpacing: 1.2, color: Colors.grey[600])),
                  Spacer(),
                  if (trailing != null)
                    Text(trailing, style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.bold, color: color)),
                ],
              ),
  ...
  Widget _buildPlotTypeChips() {
    final typesToShow = _showAllPlotTypes ? widget.controller.plotTypes : widget.controller.plotTypes.take(_initialItemCount).toList();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Wrap(
    ...
    return Container(
      ...
      child: SliderTheme(
        data: SliderTheme.of(context).copyWith(
          ...
          rangeThumbShape: RoundRangeSliderThumbShape(enabledThumbRadius: 12.r, elevation: 5),
        ),
        child: child,
      ),
    );
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Icon(Icons.refresh_rounded, color: Colors.red[400], size: 22.w),
            ),
          ),
        ],
      ),
